<?php
    ...
    try{
        ...
        for ($cont = 0; $cont < count($arquivo['name']); $cont++) {
            ...
            $destinopasta = "../../view/eventos/" . $arquivo['name'][$cont];
            $destinobanco = "eventos/" . $arquivo['name'][$cont];
            $nome = $arquivo['name'][$cont];

            if (move_uploaded_file($arquivo['tmp_name'][$cont], $destinopasta)) {

                $query = "INSERT INTO imagem_evento (id_imagem, titulo, data_cadastro, caminho) VALUES (null, '$nome' , now(), '$destinobanco')";
                $banco->exec($query);
                ...
                echo" <script>document.location.href='../../view/cadastrar_img_evento.php'</script>";
