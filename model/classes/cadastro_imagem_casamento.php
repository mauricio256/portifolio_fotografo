<?php
    ...
    try{
        ...
        for ($cont = 0; $cont < count($arquivo['name']); $cont++) {
            ...
            $destinopasta = "../../view/casamentos/" . $arquivo['name'][$cont];
            $destinobanco = "casamentos/" . $arquivo['name'][$cont];
            $nome = $arquivo['name'][$cont];

            if (move_uploaded_file($arquivo['tmp_name'][$cont], $destinopasta)) {

                $query = "INSERT INTO imagem_casamento (id_imagem, titulo, data_cadastro, caminho) VALUES (null, '$nome' , now(), '$destinobanco')";
                $banco->exec($query);
                ...
                echo" <script>document.location.href='../../view/cadastrar_img_casamento.php'</script>";
